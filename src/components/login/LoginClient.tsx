...
export function LoginClient() {
	...
	const returnTo = useMemo(
		() => safeReturnTo(searchParams.get("return_to") ?? undefined, "/"),
		[searchParams],
	);
	...
	useEffect(() => {
		...
		getOptionalCurrentUserClient().then((nextUser) => {
			...
			setUser(nextUser);
			setLoading(false);
			if (nextUser && searchParams.get("autoredirect") === "1")
			...
			<section className="hero">
				...
				>
					...
					{loading ? (
						...
						<>
							<p>
								현재 <b>{user.github_login ?? user.display_name ?? "사용자"}</b>{" "}
								계정으로 로그인되어 있습니다.
							</p>
							<LogoutButton />
							<p>
								<Link href={returnTo}>돌아가기</Link>
							</p>
						</>
					) : (
						<>
							<p className="muted">
								GitHub 계정으로 로그인하여 취약점 분석을 시작하세요.
							</p>
							<a className="primary-button" href={loginUrl}>
								GitHub 로그인
							</a>
							<p className="muted">
								GitHub 로그인 후 소스코드 분석 기능을 이용할 수 있습니다.
							</p>
						</>
					)}
				</div>
