  ...
  return (
    <header className="dashboard-hero-card">
      ...
      <div className="dashboard-hero-actions" aria-label="상세 분석 작업">
        <Button className="dashboard-primary-action" nativeButton={false} render={<Link href={buildDashboardHref(repo, analysisId)} />}>
          대시보드 보기
        </Button>
...
function GuideDistributionCard({ items, maxCount }: { items: GuideDistributionItem[]; maxCount: number }) {
  return (
          ...
          {items.map((item) => (
            <div className="bar-row" key={item.category}>
              ...
              <div aria-hidden="true">
                <i style={{ width: `${Math.round((item.count / maxCount) * 100)}%` }} />
              </div>
              <b>{item.count}</b>
            </div>
          ))}
...
function CallGraphCard({ callGraph }: { callGraph: AnalysisDetailViewModel['call_graph'] }) {
  ...
  return (
    <Card className="dashboard-card">
      <CardContent className="dashboard-card-content">
        <div className="dashboard-card-header">
          <span>호출 그래프</span>
          <Badge variant="outline">노드 {callGraph.node_count.toLocaleString()} · 엣지 {callGraph.edge_count.toLocaleString()}</Badge>
        </div>
        ...
        <div className="call-preview">
          {callGraph.preview.length ? (
            ...
            <span className="dashboard-muted">표시할 호출 그래프 미리보기가 없습니다.</span>
          )}
        </div>
      </CardContent>
    </Card>
...
function LlmReportCard({ report }: { report: AnalysisDetailViewModel['llm_report'] }) {
  if (report.text) {
    return (
      <Card className="dashboard-card llm-report">
        <CardContent className="dashboard-card-content">
          <div className="llm-report-header">
            <span>LLM 보안 리포트</span>
  ...
  return (
    <Card className="dashboard-card llm-report unavailable">
      <CardContent className="dashboard-card-content">
        ...
        <p>{report.error ?? 'LLM 리포트 생성에 실패했습니다.'}</p>
      </CardContent>
    </Card>
  );
}

function VulnerabilityGroups({ grouped }: { grouped: [string, VulnerabilityDetail[]][] }) {
  if (!grouped.length) {
    return (
      <Card className="dashboard-card">
        <CardContent className="dashboard-card-content empty-row">발견된 취약점이 없습니다.</CardContent>
      </Card>
    );
  }
  ...
  return grouped.map(([type, items]) => (
    <details className="vgroup analysis-vgroup" key={type}>
      <summary>
        <b>{type}</b>
        <span className={`level-badge level-${items[0].severity}`}>{items[0].severity}</span>
        <span>총 {items.length}건 발견됨</span>
      </summary>

      {items.map((detail) => (
        <VulnerabilityArticle detail={detail} key={detail.id || `${detail.file}-${detail.line}-${detail.type}`} />
      ))}
    </details>
  ));
}
  ...
  return (
    <header className="vitem-summary">
      <div className="vitem-header">
        ...
        <span>
          Line {detail.line ?? '-'} <b className={`level-badge level-${detail.severity}`}>{detail.severity}</b>
        </span>
      </div>
  ...
  return (
    <section className="vitem-code-compare" aria-label="취약 코드와 권장 수정 예시">
      <div className="code-panel code-panel-vulnerable">
        ...
        <pre className="vitem-code">{highlightedLines.map((line) => (
